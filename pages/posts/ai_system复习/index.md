...
def index_remove(tensor, dim, index):
    ...
    # 示例：用集合差得到剩下的索引（注意：不能把值赋给表达式）
    # example = set(range(3)) - {1}  # example == {0, 2}

    # 正确的 selectIndex 写法（把集合差转换为有序列表，按需要排序）
    selectIndex = list(set(range(tensor.size(dim))) - set(index))
    selectIndex.sort()  # 如需按升序保留顺序

    # 根据 selectIndex 构造 tensor 索引（注意 torch.index_select 需要 LongTensor）
    new_tensor = torch.index_select(tensor, dim, torch.tensor(selectIndex, dtype=torch.long))
    return new_tensor
```
